    ...
    <div id="cf7mls_multi_step_wrap" class="cf7mls_multi_step_wrap">
        ...
        <fieldset class="cf7mls-group-color">
				...
				<div class="cf7mls-transition-effects-wrap">
					<h2><?php echo _e('Transition Effects', 'cf7mls'); ?></h2>
					<div class="cf7mls-wrap-switch">
							<label class="cf7mls_switch">
						...
						<div class="cf7mls_select_stype_transition">
								<select name="cf7mls_select_stype_transition" id="cf7mls_select_stype_transition">
									<?php
										...
										$stype_transitions = [ 
												'in_fadeIn next_fadeInRight back_fadeInLeft' => 'In: Fade In, Next: Fade In Left, Back: Fade In Right', 
												'in_fadeInUp next_fadeInUp back_fadeInUp'=>'In: Fade In Up, Next: Fade In Up, Back: Fade In Up'
										];
...
function cf7mls_progress_bar_func($post)
{
	...
	<div class="cf7mls_pogress_bar_wrap">
		<div class="cf7mls-group-pogress-bar"> 
			...
			<div class="cf7mls_bg_color_wrap">
			...
			<div>
				<h2><?php _e('Style', 'cf7mls'); ?></h2>

				<select name="cf7mls_progress_bar_style" id="cf7mls_progress_bar_style">
					<?php 
					foreach ($styles as $k => $style) {
						echo sprintf('<option value="%1$s" %2$s>%3$s</option>',$style, selected($style, $selected_style, false), esc_html(__($step_bar_stype[$k], 'cf7mls')));
					}
					?>
				</select>
			...
			<div>
				<h2><?php _e('Select Option', 'cf7mls'); ?></h2>
			...
			<div class="cf7mls-allow-choose-step-wrap">
				...
				<label for="cf7mls-allow-choose-step"></label>
		...
		<div class="cf7mls_browser">   
			...
			<div class="cf7mls_block">
				...
				<div class="cf7mls_check_step_progress_bar">
					...
					}else if(count($cf7mls_step_name) > 1 && count($cf7mls_step_name) >= 3) {
					...
					<ul id="cf7mls_progress_bar" data-bg-color="<?php echo ($post->prop('cf7mls_progress_bar_bg_color')? $post->prop('cf7mls_progress_bar_bg_color') : '#0073aa'); ?>"
						...
						style="<?php echo((($style_text == 'no') && (count($cf7mls_step_name) > 1))? ('width:' . $width_progress_bar) : '') ?>" 
					...
					if(
						...
						}else if(count($cf7mls_step_name) == 2){
					...
					foreach ($cf7mls_step_name as $k => $v) {
						if($k < 3 && count($cf7mls_step_name) > 1) {
							$format_step = '';
							$format_step .= '<li class="cf7_mls_steps_item" style="width : %1$s">';
...
{	
	...
	if(is_array(maybe_unserialize($properties)) && 
		array_key_exists('cf7mls_step_name',$properties) &&
		array_key_exists('form',$properties)
	) {
...
function cf7mls_save_contact_form($contact_form)
{
    ...
    if (isset($_POST['cf7mls_step_name'])) {
        $properties['cf7mls_step_name'] = maybe_serialize(cf7mls_sanitize_arr($_POST['cf7mls_step_name']));
    }
    if (isset($_POST['cf7mls_progress_bar_style'])) {
        $properties['cf7mls_progress_bar_style'] = maybe_serialize(sanitize_text_field($_POST['cf7mls_progress_bar_style']));
    }
